...
def normalize_clip_bounds(
    ...
    *,
    ...
    if not all(math.isfinite(value) for value in (raw_start, raw_end, duration, minimum)):
    ...
    elif normalized_end <= normalized_start:
        normalized_end = min(duration, normalized_start + minimum)
        if normalized_end <= normalized_start:
            normalized_end = duration
            normalized_start = max(0.0, duration - minimum)
    elif normalized_end - normalized_start < minimum:
