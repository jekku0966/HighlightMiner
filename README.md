...
> **Status:** Current actively maintained version. Development and testing happen on `v0.2-dev`; v0.1 on `main` is the original crude MVP / proof of concept and is retained mainly for history and reference.

**TL;DR:** HighlightMiner scans long VODs and ranks moments that are likely worth reviewing using audio activity, optional local Whisper speech cues, and optional chat activity. You review, retime, keep or reject the candidates and export the clips locally — it accelerates the hunt for highlights, but it does not replace human judgment or act as a full video editor.
...
The goal is simple: turn hours of VOD into a much smaller ranked list of moments a human should actually inspect.

HighlightMiner is deliberately **human-in-the-loop**. It suggests where to look first; you decide what is good, fix the clip boundaries, name it, and choose whether it gets exported.

## v0.2 at a glance

Compared with the original v0.1 MVP, v0.2 is the current application architecture and adds:
...
- same-VOD reruns with reusable expensive analysis data;
- in-app analysis settings and model-access controls;
- explicit local-model / model-download / no-transcription choices;
- persistent Keep / Reject / Unreviewed decisions;
- editable clip timing and titles;
- a persistent export queue and export history;
- analysis deletion with impact preview and explicit confirmation;
- improved diagnostics, timing, cancellation, and shutdown behavior;
- data plumbing for future preference learning without letting a model override the user.

v0.2 is still under active testing, so bugs and rough edges are possible. That is what the `-dev` branch name is for; it is nevertheless the version that represents HighlightMiner today.

## Windows desktop UI

On Windows, Streamlit is hosted inside a native HighlightMiner window using pywebview + Microsoft Edge WebView2. Running `HighlightMiner.exe` opens the application directly instead of requiring a normal browser tab.
...
Closing the native window or using **Exit HighlightMiner** shuts down the local Streamlit process when it is safe to do so. Active non-cancellable analysis/export stages keep shutdown locked until they reach a safe state.
...
- FFmpeg handles media extraction/export;
- Whisper runs locally through `faster-whisper`/CTranslate2 when speech recognition is enabled;
- analysis/review state is stored locally in SQLite;
- no cloud inference API is required for the normal workflow.

For Twitch VOD/chat acquisition, [TwitchDownloader](https://github.com/lay295/TwitchDownloader) is the recommended companion tool. It is not bundled with or automatically invoked by HighlightMiner.
...
- transcript/audio/chat features;
...
Completed analyses are retained as history entries. A selected analysis can be permanently deleted only after an impact preview and explicit confirmation. Other runs remain untouched, source run numbers are not reused, and already-exported video files are not silently deleted from disk.
...
- **Analyze again** — create a new run while reusing compatible expensive data;
- **Force full reprocess** — deliberately rebuild the analysis from scratch.
...
4. or **Continue without speech** and complete the analysis with the signals that remain available.
...
The CLI is intentionally non-interactive for model consent. For a missing model, choose explicitly:
...
Signal presets include **Balanced**, **Reaction-heavy**, **Chat-heavy**, and **Audio-heavy**. Unavailable signals receive zero effective weight and the remaining available signals are renormalized automatically.

Saved settings affect future analyses/reruns. Existing analyses retain their original settings snapshot.

See [`SETTINGS.md`](SETTINGS.md) for the complete settings contract.

## Review and export

The Mine / Review workflow provides:

- local VOD/chat/work-folder selection;
...
- transcript/signal context;
- persistent export queue.

Kept clips can be staged before FFmpeg starts. Queue entries survive Streamlit reruns, reject duplicates, expose per-item and overall status, retain failures for retry, and record successful outputs in export history.
...
Candidate feature snapshots, ranking scores, content/game labels, source/run IDs, settings snapshots, review changes, timing edits, titles, exports, signal availability, and effective scoring weights are retained so future preference-learning experiments have usable data.
...
v0.2 can import an old v0.1 `analysis.json` and, when available, its companion review/transcript/audio/chat data into SQLite. The referenced source VOD must still exist locally.

The v0.1 branch itself remains available on [`main`](https://github.com/jekku0966/HighlightMiner/tree/main), but it should now be treated as the legacy MVP rather than the current HighlightMiner experience.
...
Ordinary GitHub Actions runs can build and smoke-test the frozen Windows application for regression coverage, but they do **not** automatically make every CI build an official downloadable release.
...
Third parties can build the open-source application themselves, but those builds are not official HighlightMiner binaries unless they are published by the maintainer as release assets.
...
- [`ATTRIBUTIONS.md`](ATTRIBUTIONS.md) — dependency/provenance notes
